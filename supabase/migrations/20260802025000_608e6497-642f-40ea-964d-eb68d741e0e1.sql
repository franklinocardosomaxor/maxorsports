-- RLS já vem habilitada em storage.objects; faltavam políticas.
-- Regra: só quem tem cargo no CRM (linha em public.user_roles) manipula
-- arquivos do bucket privado product-images. O site público serve as imagens
-- via proxy com a chave de serviço, que ignora RLS.
...
REVOKE ALL ON FUNCTION public.is_crm_member(uuid) FROM PUBLIC, anon;
...
USING (bucket_id = 'product-images' AND public.is_crm_member(auth.uid()));
...
WITH CHECK (bucket_id = 'product-images' AND public.is_crm_member(auth.uid()));
